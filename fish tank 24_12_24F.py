...
# Total vertical depth and horizontal width in cm
total_depth_cm = frame_height / CM_TO_PIXELS_RATIO
total_width_cm = frame_width / CM_TO_PIXELS_RATIO

# Display the total dimensions of the fish tank
...
while True:
    ...
    if contours:
        ...
        if previous_position is not None:
            ...
            speed_cm_per_sec = (distance / CM_TO_PIXELS_RATIO) / frame_time  # Speed in cm/sec
...
# Save vertical depth histogram
if vertical_positions_cm:
    plt.figure(figsize=(10, 6))
    plt.hist(vertical_positions_cm, bins=20, orientation='horizontal', color='blue', edgecolor='black', alpha=0.7)
    plt.title("Vertical Depth Frequency of Fish")
    plt.xlabel("Frequency")
    plt.ylabel("Depth (cm)")
    plt.ylim(0, total_depth_cm)  # Set y-axis range to match total tank depth
    plt.gca().invert_yaxis()  # Invert y-axis so 0 is at the top
    plt.grid(axis='x', alpha=0.75)

    # Save the histogram as a PNG file
    histogram_path = os.path.join(output_dir, "vertical_depth_frequency.png")
    plt.savefig(histogram_path)
    plt.show()  # Display the graph
    print(f"Vertical depth histogram saved to {histogram_path}")
else:
    print("No vertical positions were recorded.")

# Create a table with metrics
data = {
    "Metric": [
        "Total depth of the fish tank",
        "Total width of the fish tank",
        "Grid cell size",
    ...
    "Centimeters": [
        ...
        total_width_cm,
        grid_cell_size_cm,
        np.mean(vertical_positions_cm) if vertical_positions_cm else 0,
        mode(vertical_positions_cm) if vertical_positions_cm else 0,
        total_distance_cm,
        average_speed,
        max_speed
    ]
...
table_path = os.path.join(output_dir, "fish_metrics_table.csv")
df.to_csv(table_path, index=False)

# Display the table
print("\nDisplaying Fish Metrics Table:")
...
csv_file_path = os.path.join(output_dir, "fish_tracking_metrics.csv")
metrics_df.to_csv(csv_file_path, index=False)

# Output the CSV file path for confirmation
print(f"Metrics CSV file created at: {csv_file_path}")